...
public class WorkOrderController {
    ...
    public ResponseEntity<ApiResponse<List<WorkOrderResponse>>> getByWard(@RequestParam String ward) {
        return ResponseEntity.ok(ApiResponse.<List<WorkOrderResponse>>builder()
                .success(true).message("Public works orders fetched successfully")
                .data(workOrderService.getAll(null, null, ward)).build());
    }

    @GetMapping("/public/{workOrderId}")
    @Operation(summary = "Get public work order details (Citizen)")
    public ResponseEntity<ApiResponse<WorkOrderResponse>> getPublicById(@PathVariable Long workOrderId) {
        return ResponseEntity.ok(ApiResponse.<WorkOrderResponse>builder()
                .success(true).message("Work order details fetched successfully")
                .data(workOrderService.getById(workOrderId)).build());
    }
    ...
    public ResponseEntity<ApiResponse<List<WorkOrderResponse>>> getAll(
        ...
        return ResponseEntity.ok(ApiResponse.<List<WorkOrderResponse>>builder()
                .success(true).message("Work orders fetched successfully")
                .data(workOrderService.getAll(status, category, ward)).build());
    }
    ...
    public ResponseEntity<ApiResponse<WorkOrderResponse>> getById(@PathVariable Long workOrderId) {
        return ResponseEntity.ok(ApiResponse.<WorkOrderResponse>builder()
                .success(true).message("Work order details fetched successfully")
                .data(workOrderService.getById(workOrderId)).build());
    }
